...
var sockId;
sockId = tcp_connect(argument0, 80);
...
write_string(sockId, "GET " + argument1 + " HTTP/1.0" + CRLF);
write_string(sockId, "Host: " + argument0 + CRLF + CRLF);
socket_send(sockId);

// Read the entire response
var retVal;
retVal = "";
while(!tcp_eof(sockId)) {
    var buffer;
    buffer = tcp_receive_available(sockId);
    retVal += read_string(buffer, buffer_size(buffer));
    buffer_destroy(buffer);
}

// Did we read the entire response or did we error out?
if(socket_has_error(sockId)) {
    socket_destroy(sockId, false);
    exit;
}
